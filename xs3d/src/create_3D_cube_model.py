...
from .pixel_params import Rings, v_interp,eps_2_inc
from .create_dataset import dataset_to_2D_b as dataset_to_2D
...
class best_3d_model:
	def __init__(self,mommaps_obs,cube,hdr,config,vmode,V_k, pa, eps, x0, y0, Vsys, ring_pos, ring_space, pixel_scale, inner_interp, m_hrm = 1, phi_b = None, nVk=2):
		...
		self.shape2d=self.mom0.shape
		self.shape3d=self.datacube.shape		
		...
		self.Intens = tab_mod_vels(self.rings_pos,self.mommaps_obs, None, self.pa,self.eps,self.x0,self.y0,self.Vsys,0,self.ring_space,self.pixel_scale,'intensity',self.shape2d,0,0, np.inf);self.intens1d_tmp=np.copy(self.Intens)
		self.Intens=np.append(self.Intens,-1e4)

	def kinmdl_dataset(self, pars, i, xy_mesh, r_space, r_2 = None, disp = False, vconst = None):
		...
		if self.r1st !=0:	
			...
			# Intensity is also extrapolated
			if vconst is not None:
				s1, s2 = self.Intens[0], self.Intens[1]
				s_int =  v_interp(0, r2, r1, s2, s1 )
				self.Intens[self.index_v0] = s_int					



		Weights_xy=weigths_w(xy_mesh,self.pa,self.eps,self.x0,self.y0,r_2,r_space,pixel_scale=self.pixel_scale)

		Sig = self.Sig[i]
		modl0 = (SIGMA_MODEL(xy_mesh,Sig,self.pa,self.eps,self.x0,self.y0))*Weights_xy
		if disp:
			return modl0
						
		if vconst is not None:
			vval=self.Intens[i]
			mdl_const=(SIGMA_MODEL(xy_mesh,vval,self.pa,self.eps,self.x0,self.y0))*Weights_xy
			return mdl_const
		...
		if "hrm" not in self.vmode:
			Vrot = self.Vrot[i]
		if self.vmode == "circular":
			v1 = (SIGMA_MODEL(xy_mesh,Vrot,self.pa,self.eps,self.x0,self.y0))*Weights_xy
			return modl0,v1		
		if self.vmode == "radial" or self.vmode == 'vertical':
			Vrad = self.Vrad[i]
			v1 = (SIGMA_MODEL(xy_mesh,Vrot,self.pa,self.eps,self.x0,self.y0))*Weights_xy			
			v2 = (SIGMA_MODEL(xy_mesh,Vrad,self.pa,self.eps,self.x0,self.y0))*Weights_xy
			return modl0,v1,v2
		if self.vmode == 'ff':
			p = self.Vrad[i] !=0
			Vrad = Vrot*p
		...
		if self.vmode == "bisymmetric":
			...
			if Vrad != 0 and Vtan != 0:
				v1 = (SIGMA_MODEL(xy_mesh,Vrot,self.pa,self.eps,self.x0,self.y0))*Weights_xy			
				v2 = (SIGMA_MODEL(xy_mesh,Vrad,self.pa,self.eps,self.x0,self.y0))*Weights_xy				
				v3 = (SIGMA_MODEL(xy_mesh,Vtan,self.pa,self.eps,self.x0,self.y0))*Weights_xy							
			else:
				v1 = (SIGMA_MODEL(xy_mesh,Vrot,self.pa,self.eps,self.x0,self.y0))*Weights_xy
				v2=v1*0
				v3=v1*0								
		...
		if "hrm" in self.vmode:
			...
			Ck = [(SIGMA_MODEL(xy_mesh,ck,self.pa,self.eps,self.x0,self.y0))*Weights_xy for ck in c_k]
			Sk = [(SIGMA_MODEL(xy_mesh,sk,self.pa,self.eps,self.x0,self.y0))*Weights_xy for sk in s_k]						
			vels=[Ck,Sk]
	...
	def model3D(self,twoD_only=False):
			...
			mom0axi=dataset_to_2D([self.ny,self.nx], self.n_annulus, self.rings_pos, self.r_n, self.XY_mesh, self.kinmdl_dataset, None, self.v_center, None, self.index_v0, nmodls=1, vconst = self.Intens)
			...
			"""
			if self.r1st !=0:
				mask_inner = np.where( (self.r_n < self.r1st ) )
				...
				# Intensity is always extrpolated:			
				I_xy_mdl = self.kinmdl_dataset(None, v1_index, (x_r0,y_r0), r_2 = r2, r_space = r_space_0, vconst= True )
				I_new_1 = I_xy_mdl[0]
				I_xy_mdl = self.kinmdl_dataset(None, v2_index, (x_r0,y_r0), r_2 = r1, r_space = r_space_0, vconst = True)
				I_new_2 = I_xy_mdl[1]

				I_new = I_new_1 + I_new_2
				mom0axi[mask_inner] = I_new
			...
			return self.intens1d_tmp,mom0axi,mom0_mdl,mom1_mdl,mom2_mdl_kms,mom2_mdl_A,cube_mdl,velmap_intr,sigmap_intr,twoDmodels
