...
class Print:
	...
	def guess_vals(self,galaxy,guess,vmode):
		...
		INC=round(INC,2)
		PA=round(PA,2)
		print ('  ' + '{:<20}'.format(f'** Initial values for {galaxy} **'))		
	...
	def out(self,hdr,value,tab="\t"):
		print(self.deli)
		txt = 	'  ' + '{:<20} {:<15}'.format(f'{hdr}:', f'{value}')	
		#print ('  ' + '{:<20} {:<15}'.format(f'{hdr}:', f'{value}'))
		print (txt.ljust(self.n,'-'))		
				
	def long(self,text,tab="\t"):
		print(self.deli)
		txt = 	'  ' + text
		print (textwrap.fill(txt, width=self.n, subsequent_indent = '  '))		
