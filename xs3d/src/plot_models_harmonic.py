...
def plot_kin_models_h(galaxy,vmode,momms_mdls,R,Sigma,eSigma,Ck,Sk,e_Ck,e_Sk,VSYS,ext, m_hrm, out):
	...
	ax0.plot(R,Sigma, color = "#db6d52",linestyle='--', alpha = 1, linewidth=0.8, label = "$\sigma_{gas}$")
	...
	axs(ax0, remove_xticks= True, rotation = 'horizontal', fontsize_ticklabels=16)


	# plot the 1D velocities with the predefined colors
	ax1.plot(R,-1e4*Sigma, color = "#db6d52",linestyle='--', alpha = 1, linewidth=0.8, label = "$\sigma_{gas}$")
	...
	axs(ax1,  rotation = 'horizontal', fontsize_ticklabels=16)
	...
	ax1.set_xlabel(f"r ({rlabel})", labelpad = 2, fontsize = 16)
	ax0.set_ylabel("$\mathrm{Velocity~(km/s)}$",fontsize = 16,labelpad=2)
	ax1.set_ylabel("$\mathrm{V_{NC}~(km/s)}$",fontsize = 16,labelpad=2)


	ax1.legend(loc = "center", fontsize = 13, bbox_to_anchor = (0.5, 1.15), bbox_transform=ax0.transAxes, ncol = m_hrm+1, frameon = 1, labelspacing=0.1, handlelength=1, handletextpad=0.3,columnspacing=0.8)
