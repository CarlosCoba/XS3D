...
from src0.eval_tab_model import tab_mod_vels
from src0.phi_bar_sky import pa_bar_sky
from src0.fit_params import Fit_kin_mdls as fit
from src0.fit_params_boots import Fit_kin_mdls as fit_boots
from src0.tools_fits import array_2_fits
...
class Circular_model:
	...
	def lsq(self,fit_routine=fit):
		...
		for it in np.arange(self.n_it):
			...
			#plt.plot(R_pos,disp_tab,'k-');plt.plot(R_pos,vrot_tab,"r-");plt.show()
			#plt.plot(R_pos,vrot_tab,"r-");plt.plot(R_pos,vrad_tab);plt.show()
