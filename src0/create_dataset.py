...
def dataset_2_2D(shape, n_annulus, rings_pos, r_n, XY_mesh, kinmdl_dataset, vmode, v_center, pars, index_v0, V_i = [], nmodls=2 ):

	[ny, nx] = shape
	interp_model = np.zeros((ny,nx))
	sig_interp_model = np.zeros((ny,nx))	
	...
	if nmodls==2:
		return sig_interp_model,interp_model
	if nmodls==1:
		return interp_model
	if nmodls==0:
		return sig_interp_model
		
		
		


def dataset_to_2D(shape, n_annulus, rings_pos, r_n, XY_mesh, kinmdl_dataset, vmode, v_center, pars, index_v0, V_i = [], nmodls=2 ):
